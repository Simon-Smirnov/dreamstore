...
//    ];
    ...
    {
        //$storeInfo = $this->helper('storeInfo');

        $hash = hash('md5', json_encode($parameters) . date('d_m_Y'));
        if (!DEBUG) {
            $vk_review = $this->cache->get('vk_review.' . $hash);
        }
        if (!isset($vk_review)) {
            //$vk_post_ids = $this->helper('getAcf', ['key' => 'vk_always_post']);
            //if ($vk_post_ids) {
            //    $vk_post_ids = explode(',', $vk_post_ids);
            //} else {
            //    $vk_post_ids = [];
            //}

            $vk_post_ids[] = 546781926;
            $request_params = [ //подготавливаем параметры для запроса к api vk
                'group_id' => abs($storeInfo['vk_group_id']),
                'topic_id' => $storeInfo['vk_topic_id'],
                'sort' => 'desc',
                'count' => !empty($parameters['count']) ? $parameters['count'] : 20,
                'extended' => 1,
                'lang' => 'ru'
            ];
            ...
            if ($result) {
                foreach ($result['response']['items'] as $item) {
                    ...
                    $vk_review[$index]['href_post'] = "https://vk.com/topic-" . abs($storeInfo['vk_group_id']) . "_" . $storeInfo['vk_topic_id'] . "?post={$item['id']}";
