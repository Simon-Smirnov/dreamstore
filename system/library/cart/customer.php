...
class Customer
{
    ...
    {
        $query = $this->db->query("SELECT COUNT(*) as total FROM " . DB_PREFIX . "customer_total_reward WHERE customer_id = '" . (int)$this->customer_id . "'");
        if ($query->row['total'] > 0) {
            $query = $this->db->query("UPDATE " . DB_PREFIX . "customer_total_reward SET points = '" . (int)$points . "', date_update = NOW() WHERE customer_id = '" . (int)$this->customer_id . "'");
        } else {
            $this->db->query("INSERT INTO " . DB_PREFIX . "customer_total_reward SET customer_id = '" . (int)$this->customer_id . "', points = '" . (int)$points . "', date_update = NOW()");
        }
