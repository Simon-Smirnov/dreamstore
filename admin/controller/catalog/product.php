...
class ControllerCatalogProduct extends Controller
{
        ...
        foreach ($results as $result) {
            ...
            foreach ($this->model_catalog_product->getProductCategories($result['product_id']) as $category_id) {
                if (!isset($category_by_id[$category_id])) {
                    ...
                    if ($category_info) {
                        $category_by_id[$category_id] = $category_info['name'];
                    } else {
                        $category_by_id[$category_id] = false;
                    }
                }
    ...
    protected function getForm()
    {
        ...
        if (isset($this->request->post['product_set'])) {
            $productsSet = $this->request->post['product_set'];
        } elseif (isset($this->request->get['product_id'])) {
            $productsSet = $this->model_catalog_product->getProductSet($this->request->get['product_id']);
        } else {
            $productsSet = array();
        }
        ...
        foreach ($productsSet as $product_id) {
            $set_info = $this->model_catalog_product->getProduct($product_id);

            if ($set_info) {
                $data['product_sets'][] = array(
                    'product_id' => $set_info['product_id'],
                    'name' => $set_info['name']
                );
            }
        }
        ...
        $data['colors'] = $this->model_catalog_colors->getColors();

        //colors
        if (isset($this->request->post['product_color'])) {
            $data['color_id'] = $this->request->post['product_color'];
        } elseif (isset($this->request->get['product_id'])) {
            $color_info = $this->model_catalog_product->getProductColor($this->request->get['product_id']);
            $data['color_id'] = $color_info['color_id'];
        } else {
            $data['color_id'] = '';
        }
        ...
        //    $color_info = $this->model_catalog_colors->getColor($color_id);
        //
        //    if ($color_info) {
        //        $data['product_colors'][] = array(
    ...
    public function autocompleteSet()
    {
        $json = array();
