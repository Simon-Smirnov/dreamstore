...
class ControllerCatalogCategory extends Controller
{
    ...
    protected function getList()
    {
        ...
        foreach ($results as $result) {
            $data['categories'][] = array(
                ...
                'view' => HTTPS_CATALOG . 'index.php?route=product/category&path=' . $result['category_id'],
        ...
        if (isset($filter_parent)) {
            if ($filter_parent == 0) {
                $data['filter_parent_name'] = $this->language->get('text_no_parent');
            } else {
                $category_info = $this->model_catalog_category->getCategory($filter_parent);

                if ($category_info) {
                    $data['filter_parent_name'] = $category_info['name'];
                } else {
                    $data['filter_parent_name'] = '';
                    $filter_parent = null;
                }
            }
        }
    ...
    protected function getForm()
    {
        ...
        if (isset($this->error['error_tag'])) {
            $data['error_tag'] = $this->error['error_tag'];
        } else {
        ...
        if (isset($this->request->post['tag'])) {
            ...
            $data['tags'] = $this->model_catalog_category->getCategoryTags($this->request->get['category_id']);
        }
    ...
    protected function validateForm()
    {
        ...
        if (isset($this->request->post['tag'])) {
            foreach ($this->request->post['tag'] as $key => $tag) {
                if ((utf8_strlen($tag['title']) < 1) || (utf8_strlen($tag['title']) > 255)) {
                    $this->error['error_tag'][$key]['title'] = $this->language->get('error_tag_title');
                }
                if ((utf8_strlen($tag['link']) > 255)) {
                    $this->error['error_tag'][$key]['link'] = $this->language->get('error_tag_link');
                }
            }
        }
