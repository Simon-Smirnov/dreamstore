...
class ModelCatalogProduct extends Model
{
    ...
    public function getProductsByFilter($data = array())
    {
        ...
        $sql .= "SELECT p.product_id, price, p.sort_order, p.special_sort_order, p.sale_percent, (SELECT (p.price -(p.price / 100 * p.sale_percent))) AS sale_price FROM (";

        $sql .= "SELECT p.product_id, p.price AS price, p.sort_order, p.special_sort_order, p.sale_percent, (SELECT AVG(rating) AS total FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = p.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating, (SELECT price FROM " . DB_PREFIX . "product_discount pd2 WHERE pd2.product_id = p.product_id AND pd2.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND pd2.quantity = '1' AND ((pd2.date_start = '0000-00-00' OR pd2.date_start < NOW()) AND (pd2.date_end = '0000-00-00' OR pd2.date_end > NOW())) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1) AS discount, (SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special";
        ...
        if (!empty($data['filter_custom'])) {
            if (!empty($data['filter_custom']['filter_attributes'])) {
                $sql_filter_attributes = ' SELECT at0.product_id FROM ';
                $main_index = 0;

                $values_keys = [5];
                $less_keys = [7];

                foreach ($data['filter_custom']['filter_attributes'] as $key => $group) {
                    $index = 0;

                    if (in_array($key, $values_keys)) {
                        $str_atr = "`value` LIKE ";
                    } else if (in_array($key, $less_keys)) {
                        $str_atr = "`value` >= ";
                    } else {
                        $str_atr = "text LIKE ";
                    }
                    ...
                    foreach ($group as $item) {
                        ...
                        if ($is_range) {
                            if ($index === 0) {
                                $str_atr_range .= (int)$parts[0] . " AND value <= " . (int)$parts[1] . ")";
                            } else {
                                $str_atr_range .= " OR (`value` >= " . (int)$parts[0] . " AND `value` <= " . (int)$parts[1] . ")";
                            }
                        } else {
                            if ($index === 0) {
                                if (in_array($key, $less_keys)) {
                                ...
                                if (in_array($key, $less_keys)) {
                                    $str_atr .= " OR `value` >= " . $item . "";
                                } else if (in_array($key, $values_keys)) {
                                    $str_atr .= " OR `value` LIKE '%" . $item . "%'";
                                } else {
                                    $str_atr .= " OR text LIKE '%" . $item . "%'";
                                }
                            }
                        }
                ...
                $sql .= "INNER JOIN ({$sql_filter_attributes}) AS t2  ON p.product_id = t2.product_id ";
            }

            if (!empty($data['filter_custom']['filter_options'])) {
                $sql_filter_options = ' SELECT ov0.product_id FROM ';
                $main_index = 0;

                foreach ($data['filter_custom']['filter_options'] as $key => $group) {
                    $index = 0;
                    $str_option = "option_value_id = ";
                    ...
                    foreach ($group as $item) {
                        ...
                        if ($index === 0) {
                            ...
                            $str_option .= " OR option_value_id = " . (int)$item;
                        }
                ...
                $sql .= "INNER JOIN ({$sql_filter_options}) AS t3  ON p.product_id = t3.product_id ";
            }
        }
        ...
        if (isset($data['sort']) && in_array($data['sort'], $sort_data)) {
            if ($data['sort'] == 'pd.name' || $data['sort'] == 'p.model') {
                ...
                $sql .= " ORDER BY ISNULL(p.special_sort_order), p.special_sort_order ASC, special DESC, p.sale_percent DESC, p.sort_order";
            } else {
                $sql .= " ORDER BY " . $data['sort'];
            }
        } else {
        ...
        # Фильтры
        if (!empty($data['filter_custom'])) {
            if (!empty($data['filter_custom']['price']) && ($data['filter_custom']['price']['min'] or $data['filter_custom']['price']['max'])) {
                $sql .= " WHERE ";

                $min = $data['filter_custom']['price']['min'];
                $max = $data['filter_custom']['price']['max'];

                if (!empty($min) && !empty($max)) {
                    $sql .= " (price >= '" . (int)$min . "' OR sale_price >= '" . (int)$min . "') AND (price <= '" . (int)$max . "' OR sale_price <= '" . (int)$max . "')";
                } elseif (!empty($min)) {
                    $sql .= " (price >= '" . (int)$min . "' OR sale_price >= '" . (int)$min . "')";
                } elseif (!empty($max)) {
                    $sql .= " (price <= '" . (int)$max . "' OR sale_price <= '" . (int)$max . "')";
                }
            }
        }
    ...
    public function getTotalProductsByFilter($data = array())
    {
        $data['start'] = 0;
        $data['limit'] = 999;

        $products = $this->getProductsByFilter($data);
        ...
        foreach ($product_option_query->rows as $product_option) {
            ...
            foreach ($product_option_value_query->rows as $product_option_value) {
                $product_option_value_data[] = array(
                    ...
                    'attention' => $product_option_value['attention'],
    ...
    public function getProductSet($product_id)
    {
        $product_data = array();
    ...
    public function getProductRelatedByCategory($category_id, $current_product_id)
    {
        $product_data = array();
        ...
        if ($query->num_rows > 0) {
            foreach ($query->rows as $result) {
                if ($result['product_id'] != $current_product_id) {
                    $product_data[$result['product_id']] = $this->getProduct($result['product_id']);
                }
            }
        }
