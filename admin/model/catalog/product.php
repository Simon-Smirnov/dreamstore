...
class ModelCatalogProduct extends Model
{
    ...
    {
        $this->db->query("INSERT INTO " . DB_PREFIX . "product SET model = '" . $this->db->escape($data['model']) . "', sku = '" . $this->db->escape($data['sku']) . "', upc = '" . $this->db->escape($data['upc']) . "', ean = '" . $this->db->escape($data['ean']) . "', jan = '" . $this->db->escape($data['jan']) . "', isbn = '" . $this->db->escape($data['isbn']) . "', mpn = '" . $this->db->escape($data['mpn']) . "', location = '" . $this->db->escape($data['location']) . "', quantity = '" . (int)$data['quantity'] . "', minimum = '" . (int)$data['minimum'] . "', subtract = '" . (int)$data['subtract'] . "', size_table = '" . (int)$data['size_table'] . "', stock_status_id = '" . (int)$data['stock_status_id'] . "', date_available = '" . $this->db->escape($data['date_available']) . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', shipping = '" . (int)$data['shipping'] . "', price = '" . (float)$data['price'] . "', points = '" . (int)$data['points'] . "', weight = '" . (float)$data['weight'] . "', weight_class_id = '" . (int)$data['weight_class_id'] . "', length = '" . (float)$data['length'] . "', width = '" . (float)$data['width'] . "', height = '" . (float)$data['height'] . "', length_class_id = '" . (int)$data['length_class_id'] . "', status = '" . (int)$data['status'] . "', no_index = '" . (int)$data['no_index'] . "', tax_class_id = '" . (int)$data['tax_class_id'] . "', sort_order = '" . (int)$data['sort_order'] . "', date_added = NOW(), date_modified = NOW()");
        ...
        foreach ($data['product_description'] as $language_id => $value) {
            $this->db->query("INSERT INTO " . DB_PREFIX . "product_description SET product_id = '" . (int)$product_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', description = '" . $this->db->escape($value['description']) . "', care = '" . $this->db->escape($value['care']) . "',  questions = '" . $this->db->escape($value['questions']) . "', tag = '" . $this->db->escape($value['tag']) . "', meta_h1 = '" . $this->db->escape($value['meta_h1']) . "', meta_title = '" . $this->db->escape($value['meta_title']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "'");
        }

        if (isset($data['product_colors'])) {
            $this->db->query("INSERT INTO `" . DB_PREFIX . "product_colors` SET `product_id` = '" . (int)$product_id . "', `color_id` = '" . (int)$data['product_colors'] . "'");
        }
        ...
        if (isset($data['product_set'])) {
            foreach ($data['product_set'] as $set_id) {
                $this->db->query("REPLACE INTO " . DB_PREFIX . "product_set SET product_id = '" . (int)$product_id . "', set_id = '" . (int)$set_id . "'");
            }
        }
    ...
    public function editProduct($product_id, $data)
    {
        ...
        $this->db->query("DELETE FROM " . DB_PREFIX . "product_set WHERE product_id = '" . (int)$product_id . "'");

        if (isset($data['product_set'])) {
            foreach ($data['product_set'] as $set_id) {
                $this->db->query("INSERT INTO " . DB_PREFIX . "product_set SET product_id = '" . (int)$product_id . "', set_id = '" . (int)$set_id . "'");
            }
        }
    ...
    public function copyProduct($product_id)
    {
        ...
        if ($query->num_rows) {
            ...
            $data['main_category'] = $this->getProductMainCategory($product_id);
    ...
    protected function sqlFilter($data)
    {
        ...
        // Для обратной совместимости модели
    ...
    {
        $sql = "SELECT p.*, pd.name, m.name AS manufacturer FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "manufacturer m ON (p.manufacturer_id = m.manufacturer_id)";
    ...
    public function getProductSet($product_id)
    {
        ...
        foreach ($query->rows as $result) {
            $product_set_data[] = $result['set_id'];
        }
