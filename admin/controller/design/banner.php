		...
		foreach ($banner_images as $key => $value) {
			foreach ($value as $banner_image) {
				...
				$data['banner_images'][$key][] = array(
					...
					'is_button'   => $banner_image['is_button'],
		...
		if (isset($this->request->post['banner_image'])) {
			foreach ($this->request->post['banner_image'] as $language_id => $value) {
				foreach ($value as $banner_image_id => $banner_image) {
					if ((utf8_strlen($banner_image['title']) < 2) || (utf8_strlen($banner_image['title']) > 128)) {
						$this->error['banner_image'][$language_id][$banner_image_id] = $this->language->get('error_title');
					}
					if ((utf8_strlen($banner_image['description']) < 10) || (utf8_strlen($banner_image['description']) > 1000)) {
						$this->error['banner_image'][$language_id][$banner_image_id] = $this->language->get('error_description');
					}
				}
			}
		}
